// ply.cpp : This file contains the 'main' function. Program execution begins and ends there.
//
...
int main() {
    try {
        auto arrayBuffer = loadFileAsArrayBuffer("../../pc_short.ply");
        PackedGaussians packedGaussians(arrayBuffer);

        std::cout << "Number of Gaussians: " << packedGaussians.numGaussians << "\n";
        std::cout << "Spherical Harmonics Degree: " << packedGaussians.sphericalHarmonicsDegree << "\n";
...
int main() {
    ...
    if (hFind == INVALID_HANDLE_VALUE) {
        std::wcerr << L"FindFirstFile failed (" << GetLastError() << L")" << std::endl;
        return 1;
    }
